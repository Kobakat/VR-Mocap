	...
	public class TakeScreenshot : MonoBehaviour
	{
		...
		Vector3 CalcViewBox(Vector3 center, Vector3 up, Vector3 forward)
		{
			...
			*/
			...
			var right = Quaternion.Euler(up * 90) *
			            forward; //compute a right vecotr orthagonal to the camera and scene object facing vector
			...
			return
				new Vector3(maxX, maxY,
					maxZ); //the vectors local to camera which represent the visible bounds for the rotated object(s)
		}
		...
		public void SetFrustumToSceneObject(Transform sceneObject)
		{
			...
			Vector3 refPos =
				sceneObject.transform.position + fwd * closestDistance; // m_camera.transform.parent.position;
			...
			var newZ = distance * 1f *
			           Mathf.Abs(Vector3.Dot(fwd, refPos)); // 3f;// Mathf.Max(distance * 3f, closestDistance);

			m_camera.transform.position = new Vector3(xyCP.x, xyCP.y, -newZ);
			m_camera.nearClipPlane =
				0.1f; // Mathf.Max(Vector3.Distance (m_camera.transform.position, closestPoint) - m_camera.fieldOfView * 0.1f, 0.1f);
			m_camera.farClipPlane =
				1000f; // Mathf.Max(Vector3.Distance (m_camera.transform.position, farthestPoint) + m_camera.fieldOfView * 0.1f, 1.0f);
