...
using UnityEngine.Rendering;
	...
	{
		public static string BUILDER_VERSION = "3.1.8";

		[Header("References")] Texture2D logotex;

		public static string[] getCharacterTypesArray = System.Enum.GetNames(typeof(ArmatureLinker.CharacterType));

		//============================================
		[MenuItem("APS_SDK/Avatar Importer")]
		static void Init()
		{
			AssetBuilder window = (AssetBuilder) GetWindow(typeof(AssetBuilder));
		...
		void OnEnable()
		{
			string[] tex = AssetDatabase.FindAssets("logotex", new[] {"Assets/APS_SDK"});
			if (tex.Length != 0)
			{
				byte[] fileData;
			...
			if (!Directory.Exists(APS.Automation.AssetPostprocessor.assetBundlesFolder))
			{
				Directory.CreateDirectory(APS.Automation.AssetPostprocessor.assetBundlesFolder);
			}
		...
		/// <summary>
		/// Simply sets all blendshapes weights for all facerenderers in the scene to 0
		/// </summary>
		public static void ResetAllBlendshapesToZero()
		{
			foreach (var linker in FindObjectsOfType<ArmatureLinker>()) {
				if (linker.faceRenderer != null && linker.faceRenderer.sharedMesh != null)
					for (int i = 0; i < linker.faceRenderer.sharedMesh.blendShapeCount; i++)
						linker.faceRenderer.SetBlendShapeWeight(i, 0);
			
				if (linker.jaw)
				{
					linker.jaw.localRotation = linker.iJawRotation;
					linker.jaw.localPosition = linker.iJawPosition;
				}
			}
		}

		public static void FixPlayerSettings()
		{
		...
		public static void FixShaderStripping()
		{
			var graphicsSettingsObj = AssetDatabase.LoadAssetAtPath<GraphicsSettings>("ProjectSettings/GraphicsSettings.asset");
			var serializedObject = new SerializedObject(graphicsSettingsObj);

			var field = serializedObject.FindProperty("m_InstancingStripping"); //Be sure to strip unused shaders, but include the used ones.
			field.intValue = 0;
			field = serializedObject.FindProperty("m_LightmapStripping");
			field.intValue = 0;
			field = serializedObject.FindProperty("m_FogStripping");
			field.intValue = 0;
		...
		//	customLabel.normal.textColor = Color.black;
			...
			GUILayout.Label(string.Format("Version: {0} (LUXOR)", BUILDER_VERSION /*Application.version*/),
				customLabel);
			...
			if (GUILayout.Button(
				new GUIContent("Import Avatar Model",
					"Select .blend files containing valid models. Processes the model to generate a custom assetbundle compatible with AnimationPrepStudio runtime."),
				customLabel))
			{
				...
				try
				{
					//Clear Console Window - http://answers.unity.com/answers/1318323/view.html
				...
				{
					var enumName = System.Enum.GetName(typeof(ArmatureLinker.CharacterType),
						ArmatureLinker.CharacterType.DEFAULT);
					Debug.LogWarning(
						string.Format("You must specify a character type (\"{0}\" is not a valid selection)",
							enumName));
					EditorUtility.DisplayDialog("Error",
						string.Format(
							"Character type: \"{0}\" is not a valid selection!\n\nPlease select a desired character type first.",
							enumName),
						"Ok");
				...
				if (!string.IsNullOrEmpty(blenderAppPath) && !File.Exists(blenderAppPath))
				{
					EditorUtility.DisplayDialog("Blender Application Is Not Set",
						"Please browse for and select the installed Blender application.", "OK");
					return;
				}
				...
				var modelPath = EditorUtility.OpenFilePanel(
					string.Format("Load {0} model", getCharacterTypesArray[characterType].ToLower()), modelPathLast,
					"blend");
				...
				if (!string.IsNullOrEmpty(modelPath))
				{
					...
					{
						if (!RunBatch(APS.Automation.AssetPostprocessor.AssetBundleVariant, modelPath, blenderAppPath))
						{
						...
						modelPath = Path.Combine(
							Path.Combine(
								...
								baseName.ToLower() + string.Format("_{0}",
									APS.Automation.AssetPostprocessor.AssetBundleVariant.ToLower())
							),
							baseName + ".fbx"
						);
					}
					...
					var processingPath =
						APS.Automation.AssetPostprocessor.processingFolder; // Path.Combine(Application.dataPath, "MakeHumanModels");
					...
					string uid =
						uploadName.Replace(APS.Automation.AssetPostprocessor.templateSeperator.ToString(),
							""); // uploadFolderTop.Replace("$", "");
					...
					foreach (FileInfo f in modelsInfo)
					{
						var to = Path.Combine(
							processingPath,
							APS.Automation.AssetPostprocessor.AssetBundleVariant +
							APS.Automation.AssetPostprocessor.templateSeperator + //"$"
							uid +
							APS.Automation.AssetPostprocessor.templateSeperator + //"$"
							f.Name
						);
					...
					APS.Automation.AssetPostprocessor.AssetBundleUserJson userPrefs = new APS.Automation.AssetPostprocessor.AssetBundleUserJson()
					{
						created = System.DateTime.UtcNow,
						variantTag =
							getCharacterTypesArray[
								characterType], //use the index of the users selection to get then enum name from the chracter types
						//user = userName,
					...
					var jsonPath = Path.Combine(APS.Automation.AssetPostprocessor.assetBundlesFolder, uid + ".json");
					...
					Debug.Log(string.Format(
						"Created new {0} character!\nPress \"Append Prefabs To Scene\" button to see the newly created character.",
						getCharacterTypesArray[characterType].ToLower()));
				}
			}
			...
			characterType =
				EditorGUILayout.Popup(
					new GUIContent("Character Type:",
						"This must match the type of skeleton that the character model was created with."),
					characterType, getCharacterTypesArray, GUILayout.MinWidth(100));
			...
			customLabel.alignment = TextAnchor.MiddleCenter;
			customLabel.fontSize = 14;
			customLabel.normal.textColor = new Color(0.529f, 0.737f, 0.250f);
			customLabel.fontStyle = FontStyle.Bold;

			EditorGUILayout.LabelField("Save changes to assetbundles:");

			if (GUILayout.Button(
				new GUIContent("Re-Build Assetbundles",
					"After making any changes to avatars, this will update the changes to the local assetbundle files."),
				customLabel))
			{
				...
				foreach (var armatureLinker in FindObjectsOfType<ArmatureLinker>())
					ArmatureLinker.RemoveMissingScripts(armatureLinker.transform.gameObject);

				var allPaths = AssetDatabase.GetAllAssetPaths();
				foreach (var assetPath in allPaths)
				{
					if (assetPath.StartsWith(APS.Automation.AssetPostprocessor.prefabsFolder))
					{
				...
				foreach (var go in allRootGos)
				{
					...
					bool isPrefabInstance = PrefabUtility.GetCorrespondingObjectFromOriginalSource(go) != null &&
					                        PrefabUtility.GetCorrespondingObjectFromOriginalSource(go.transform) !=
					                        null;
					if (isPrefabInstance)
					{
						...
						{
							Debug.LogWarning(e
								.Message); //Ensure all assets will be processed. Ignores error: "Given path is not valid: 'Assets/Resources/armature_something.fbx"
						}

						try
						{
							var armatureLinker = go.GetComponentInChildren<ArmatureLinker>();
							if (armatureLinker)
							{

								string assetDestFolder = Path.Combine(APS.Automation.AssetPostprocessor.assetBundlesFolder,
									armatureLinker.name.ToLower());
								...
								foreach (var renderer in armatureLinker.GetComponentsInChildren<Renderer>())
								foreach (var material in renderer.sharedMaterials)
								{
									Shader shader = material.shader;
									for (int i = 0; i < ShaderUtil.GetPropertyCount(shader); i++)
									{
										if (ShaderUtil.GetPropertyType(shader, i) ==
										    ShaderUtil.ShaderPropertyType.TexEnv)
										{
											Texture texture =
												material.GetTexture(ShaderUtil.GetPropertyName(shader, i));
								...
								var fbxPath =
									AssetDatabase.GetAssetPath(armatureLinker.GetComponent<Animator>().avatar);

								var to = Path.Combine(fbxFolder, armatureLinker.name + Path.GetExtension(fbxPath));
						...
						catch
						{
							Debug.LogError(
								go +
								" faild to create the model_data folder for the assetbundle. The .fbx was not copied.");
						}
					}
				}
			...
			if (GUILayout.Button(
				new GUIContent("Append Prefabs To Scene", "Places all prefabs into the current scene."), customLabel))
			{

				//Vector3 offsetSpacing = Vector3.right * 1.5f;

				var info = new DirectoryInfo(APS.Automation.AssetPostprocessor.prefabsFolder);
				...
				for (int i = 0; i < fileInfo.Length; i++)
				{
					...
					{
						//bool isPrefabInstance = PrefabUtility.GetPrefabParent(go) != null && PrefabUtility.GetPrefabObject(go.transform) != null;
						bool isPrefabInstance = PrefabUtility.GetCorrespondingObjectFromSource(go) != null &&
						                        PrefabUtility.GetCorrespondingObjectFromSource(go.transform) != null;
			...
			if (GUILayout.Button(
				new GUIContent("Assetbundles", "Opens the folder containing the newly created Assetbundle files."),
				customLabel))
			{
				ShowAssetBundlesExplorer();
			}

			customLabel.normal.textColor = new Color(1.0f, 0.5f, 0.2f);
			if (GUILayout.Button(
				new GUIContent("VR_MocapAssets",
					"This is the destination folder where you should copy Assetbundles to for them to be included in AnimationPrepStudio runtime."),
				customLabel))
			{
			...
			GUILayout.BeginHorizontal();
			EditorGUILayout.LabelField("Blender Executable:", GUILayout.MinWidth(0));
			...
			if (GUILayout.Button(
				new GUIContent("Browse",
					"Select a valid Blender v2.79b installation path by locating and selecting the \"blender.exe\" runtime application file."),
				customLabel))
			{
				string modelPath = EditorUtility.OpenFilePanel("Blender Application (v2.79b)",
					Path.GetDirectoryName(blenderAppPath), "exe");
				if (!string.IsNullOrEmpty(modelPath))
				{
			...
			customLabel.alignment = TextAnchor.MiddleCenter;
			customLabel.fontStyle = FontStyle.Italic;
			GUILayout.Label(string.Format("{0} - {1}", Application.unityVersion, SystemInfo.operatingSystem),
				customLabel);
		...
		public static void ShowAssetBundlesExplorer()
		{
			...
			ShowExplorer(Application.dataPath + Path.Combine(string.Format("{0}..{0}", Path.DirectorySeparatorChar),
				             APS.Automation.AssetPostprocessor.assetBundlesFolder));
		}
		...
		public static void ShowMocapAssetsExplorer()
		{
			ShowExplorer(
				Environment.ExpandEnvironmentVariables(
					string.Format(
						"%userprofile%{0}appdata{0}localLow{0}Animation Prep Studios{0}LUXOR{0}VR_MocapAssets{0}",
		...
		{
			var path = "Assets\\APS_SDK\\AssetCreator.exe";
			if (!File.Exists(path))
			{
	...
	public class BuildScript
	{
		public static void BuildAssetBundles()
		{
			...
			BuildPipeline.BuildAssetBundles(APS.Automation.AssetPostprocessor.assetBundlesFolder, BuildAssetBundleOptions.None,
				BuildTarget.StandaloneWindows);
		}

		public static void BuildAssetBundles(AssetBundleBuild[] assetBundleBuilds)
		{
			BuildPipeline.BuildAssetBundles(APS.Automation.AssetPostprocessor.assetBundlesFolder, assetBundleBuilds,
