...
{
	/// <summary>
	/// Because unity assetbundles 1.0 and standard shaders do not automatically take into account the 
	/// currently set keyword, this must be done manually. This script stores the keywords that were
	/// present at the time the character was created and represents the shader's behavior for the renderer
	...
	public void StoreParams()
	{
		...
		for (int n = 0; n < mats.Length; n++)
		{
			...
			materialsParams[n] = new MaterialParams()
			{
				...
				shaderKeywordParams =
					shaderKeywordParamsList.ToArray(), // new ShaderKeywordParams[keywordsBoolean.Length],
				shaderValuesParams =
					shaderValuesParamsList.ToArray(), // new ShaderValuesParams[keywordsFloats.Length],
			};
